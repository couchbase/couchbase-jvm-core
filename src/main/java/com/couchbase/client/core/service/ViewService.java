...
public class ViewService extends PooledService {
     ...
     * @param ctx the shared context.
     */
    public ViewService(final String hostname, final String bucket, final String username,
        final String password, final int port, final CoreContext ctx) {
        super(hostname, bucket, username, password, port, ctx, ctx.environment().viewServiceConfig(),
            FACTORY, STRATEGY);
    }
    ...
    static class ViewEndpointFactory implements EndpointFactory {
        ...
        public Endpoint create(final String hostname, final String bucket, final String username,
            final String password, final int port, final CoreContext ctx) {
            return new ViewEndpoint(hostname, bucket, username, password, port, ctx.environment(),
