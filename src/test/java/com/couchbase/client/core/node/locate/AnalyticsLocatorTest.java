...
/**
 * Verifies the functionality of the {@link AnalyticsLocator}.
 *
 * @author Michael Nitschinger
 * @since 1.5.5
 */
public class AnalyticsLocatorTest {

    @Test
    public void shouldSelectNextNode() throws Exception {
        Locator locator = new AnalyticsLocator(0);

        GenericAnalyticsRequest request = mock(GenericAnalyticsRequest.class);
        ClusterConfig configMock = mock(ClusterConfig.class);
        List<Node> nodes = new ArrayList<Node>();

        Node node1Mock = mock(Node.class);
        when(node1Mock.serviceEnabled(ServiceType.ANALYTICS)).thenReturn(true);
        when(node1Mock.hostname()).thenReturn(NetworkAddress.create("192.168.56.101"));
        Node node2Mock = mock(Node.class);
        when(node2Mock.serviceEnabled(ServiceType.ANALYTICS)).thenReturn(true);
    ...
    public void shouldSkipNodeWithoutServiceEnabled() throws Exception {
        ...
        Node node1Mock = mock(Node.class);
        when(node1Mock.hostname()).thenReturn(NetworkAddress.create("192.168.56.101"));
        when(node1Mock.serviceEnabled(ServiceType.ANALYTICS)).thenReturn(false);
        Node node2Mock = mock(Node.class);
        when(node2Mock.hostname()).thenReturn(NetworkAddress.create("192.168.56.102"));
        when(node2Mock.serviceEnabled(ServiceType.ANALYTICS)).thenReturn(false);
        Node node3Mock = mock(Node.class);
        when(node3Mock.hostname()).thenReturn(NetworkAddress.create("192.168.56.103"));
        when(node3Mock.serviceEnabled(ServiceType.ANALYTICS)).thenReturn(true);
    ...
    public void shouldDistributeFairlyUnderMDS() throws Exception {
        ...
        when(node4Mock.serviceEnabled(ServiceType.ANALYTICS)).thenReturn(true);
